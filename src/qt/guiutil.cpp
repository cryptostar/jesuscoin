...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("jesuscoin"))
        return false;
...
{
    // Convert jesuscoin:// to jesuscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("jesuscoin://"))
    {
        uri.replace(0, 12, "jesuscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "JesusCoin.lnk";
}
...
{
    return GetAutostartDir() / "jesuscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("JesusCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  jesuscoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("JesusCoin-Qt"));
